...
class Runner:
    ...
    def run(self):
        ...
        # Track per-agent and average rewards
        ...
        # Per-agent tracking
        train_episode_rewards_per_agent = [torch.zeros(1, self.config["n_rollout_threads"], device=self.config["device"]) 
                                           for _ in range(self.num_agents)]
        train_episode_costs_per_agent = [torch.zeros(1, self.config["n_rollout_threads"], device=self.config["device"]) 
                                         for _ in range(self.num_agents)]
        ...
        for episode in pbar:
            ...
            done_episodes_costs = []
            done_episodes_rewards_per_agent = [[] for _ in range(self.num_agents)]
            done_episodes_costs_per_agent = [[] for _ in range(self.num_agents)]
            ...
            for env_step in range(self.config["episode_length"]):
                ...
                if isinstance(rewards, list):
                    rewards = torch.tensor(rewards, device=self.config["device"], dtype=torch.float32).T
                if isinstance(costs, list):
                    costs = torch.tensor(costs, device=self.config["device"], dtype=torch.float32).T
                
                # Track per-agent rewards and costs
                for agent_id in range(self.num_agents):
                    train_episode_rewards_per_agent[agent_id] += rewards[:, agent_id]
                    train_episode_costs_per_agent[agent_id] += costs[:, agent_id]
                ...
                for t in range(self.config["n_rollout_threads"]):
                    if dones_env[t]:
                        ...
                        # Track per-agent episode returns
                        for agent_id in range(self.num_agents):
                            done_episodes_rewards_per_agent[agent_id].append(train_episode_rewards_per_agent[agent_id][:, t].clone())
                            done_episodes_costs_per_agent[agent_id].append(train_episode_costs_per_agent[agent_id][:, t].clone())
                        
                        episode_steps_list.append(step)
                        train_episode_rewards[:, t] = 0
                        train_episode_costs[:, t] = 0
                        for agent_id in range(self.num_agents):
                            train_episode_rewards_per_agent[agent_id][:, t] = 0
                            train_episode_costs_per_agent[agent_id][:, t] = 0
            ...
            for ep_idx, (ep_ret, ep_cost, ep_steps) in enumerate(zip(done_episodes_rewards, done_episodes_costs, episode_steps_list)):
                self.return_aver_cost(ep_cost)
                
                # Log average and per-agent metrics
                log_dict = {
                ...
                for agent_id in range(self.num_agents):
                    if ep_idx < len(done_episodes_rewards_per_agent[agent_id]):
                        log_dict[f"Metrics/Agent{agent_id}/EpRet"] = done_episodes_rewards_per_agent[agent_id][ep_idx].item()
                        log_dict[f"Metrics/Agent{agent_id}/EpCost"] = done_episodes_costs_per_agent[agent_id][ep_idx].item()
                
                self.logger.store(**log_dict)
            
            if len(done_episodes_rewards) != 0:
                # Log training metrics - average and per-agent
                ...
                for agent_id in range(self.num_agents):
                    self.logger.log_tabular(f"Metrics/Agent{agent_id}/EpRet", min_and_max=True, std=True)
                    self.logger.log_tabular(f"Metrics/Agent{agent_id}/EpCost", min_and_max=True, std=True)
                ...
                self.logger.log_tabular("Debug/Factor_mean")
                self.logger.log_tabular("Debug/Factor_std")
                self.logger.log_tabular("Debug/Factor_min")
                self.logger.log_tabular("Debug/Factor_max")
    ...
    def train(self):
        action_dim = 1
        # Initialize factor with symmetry to prevent accumulation bias
        factor = torch.ones(self.config["episode_length"], self.config["n_rollout_threads"], action_dim, device=self.config["device"])
        
        # Store log-probs of all agents BEFORE training for symmetric computation
        agent_old_logprobs = {}
        for agent_id in range(self.num_agents):
            action_dim_agent = self.buffer[agent_id].actions.shape[-1]
            available_actions = None if self.buffer[agent_id].available_actions is None \
                else self.buffer[agent_id].available_actions[:-1].reshape(-1, *self.buffer[agent_id].available_actions.shape[2:])
            
            old_logprob, _, _, _ = self.trainer[agent_id].policy.actor.evaluate_actions(
            ...
            agent_old_logprobs[agent_id] = (old_logprob, action_dim_agent)
        ...
        # SYMMETRIC FACTOR UPDATE: Use pre-stored old log-probs to prevent update-order bias
        for agent_id in torch.randperm(self.num_agents).tolist():
            action_dim = agent_old_logprobs[agent_id][1]
            ...
            old_actions_logprob = agent_old_logprobs[agent_id][0]
            ...
            # Compute importance weight ratio with proper dimensionality
        ...
        # Monitor factor symmetry and stability (for debugging)
        factor_mean = factor.mean().item()
        factor_std = factor.std().item()
        ...
        self.logger.store(
            ...
            **{
                ...
                # Debug: Monitor factor stability (NEW)
                "Debug/Factor_mean": factor_mean,
                "Debug/Factor_std": factor_std,
                "Debug/Factor_min": factor_min,
                "Debug/Factor_max": factor_max,
            }
        )

...
# ==============================================================================
"""
MAPPO-PINN-MuJoCo: MAPPO with Port-Hamiltonian Neural Network Actor for MAMujoco environments.

This algorithm integrates Physics-Informed Neural Networks (PINN) based on 
Port-Hamiltonian systems into MAPPO's actor network, specifically designed for
multi-agent MuJoCo environments (Ant, HalfCheetah, Walker2d, etc.).

Port-Hamiltonian System for Articulated Robots:
    ẋ = (J(x) - R(x)) ∇H(x) + g(x)u
    
Where:
    - x = (q, q̇) is the state (joint positions, joint velocities)
    ...
    - H(x): Hamiltonian (total energy = kinetic + potential)
    - g(x): Input matrix
    - u: Control input (joint torques)

For MAMujoco environments:
    - Ant (2x4): 8 joints total, 2 agents each controlling 4 joints
    - HalfCheetah (2x3): 6 joints total, 2 agents each controlling 3 joints
    - Observation: state + agent_id one-hot encoding (normalized)
"""
...
# =============================================================================
# Port-Hamiltonian PINN Actor for MAMujoco Environments
# =============================================================================

class PortHamiltonianMujocoActor(nn.Module):
    """
    Physics-Informed Neural Network Actor based on Port-Hamiltonian Systems
    for Multi-Agent MuJoCo environments.
    
    Port-Hamiltonian dynamics for articulated robots:
        ẋ = (J - R) ∇H(x) + g(x)u
        
    For MAMujoco agents:
        - State x includes joint positions (q) and velocities (q̇)
        - J is skew-symmetric (energy conserving interconnection)
        - R is symmetric positive semi-definite (dissipation/damping)
        - H is the Hamiltonian (kinetic + potential energy)
        - Control u is the joint torque output
        
    The network learns:
        1. H_net: Hamiltonian function H(state) -> scalar energy
        2. J_net: Skew-symmetric interconnection J(state) 
        3. R_net: Dissipation matrix R(state), enforced to be positive semi-definite
        4. Policy_net: Maps physics-informed features to action distribution
        
    MAMujoco Observation Structure (after ShareEnv processing):
        - Raw state from env.state() + agent_id one-hot (normalized)
        - Ant: 27 (13 qpos + 14 qvel) + num_agents = 29 for 2 agents
        - HalfCheetah: 17 (8 qpos + 9 qvel) + num_agents = 19 for 2 agents
    """
    
    def __init__(self, config, obs_space, act_space, device=torch.device("cpu")):
        super(PortHamiltonianMujocoActor, self).__init__()
        ...
        # Determine physics state dimension based on observation
        # MAMujoco obs = state (qpos + qvel) + agent_id_onehot, all normalized
        # We need to infer the physics state dimension
        # Common configurations:
        #   - Ant: obs_dim ~= 29 (27 state + 2 agent_id for 2 agents)
        #   - HalfCheetah: obs_dim ~= 19 (17 state + 2 agent_id for 2 agents)
        #   - Walker2d: obs_dim ~= 19 (17 state + 2 agent_id)
        #   - Hopper: obs_dim ~= 14 (11 state + 3 agent_id for 3 agents)
        
        # Estimate number of agents from obs_dim
        self.num_agents = config.get("num_agents", 2)
        self.state_dim_raw = self.obs_dim - self.num_agents  # Remove agent_id dims
        
        # For physics, we use a subset of the state (position and velocity components)
        # We'll use the first half as "positions" and second half as "velocities"
        # This is a simplification but captures the essential structure
        self.physics_state_dim = min(self.state_dim_raw, 16)  # Limit for computational efficiency
        ...
        # For articulated robots: H = T(q̇) + V(q) = 0.5 * q̇ᵀM(q)q̇ + V(q)
        self.H_net = nn.Sequential(
            nn.Linear(self.physics_state_dim, self.physics_hidden),
            nn.ELU(),
        ...
        self.J_dim = self.physics_state_dim * (self.physics_state_dim - 1) // 2
        ...
        # Models joint friction and damping
        # Output: elements of lower triangular L, then R = L @ L^T
        self.R_tril_dim = self.physics_state_dim * (self.physics_state_dim + 1) // 2
        ...
        # Base network for observation features
        ...
        # Physics features: H, grad_H (physics_state_dim)
        physics_feature_dim = 1 + self.physics_state_dim  # H + grad_H
    ...
    def _extract_physics_state(self, obs):
        """
        Extract physics-relevant state from MAMujoco observation.
        
        MAMujoco observation structure (normalized):
            - state (qpos + qvel) + agent_id one-hot
            
        We extract the first physics_state_dim elements as the physics state,
        which typically contains joint positions and velocities.
        
        Args:
            obs: [batch, obs_dim] observation tensor (already float32)
            
        Returns:
            state: [batch, physics_state_dim] physics state
        """
        # Take the first physics_state_dim elements (joint positions and velocities)
        # The observation is already normalized by ShareEnv
        state = obs[:, :self.physics_state_dim]
        return state
    ...
    def _construct_J_matrix(self, J_elements, batch_size):
        ...
        Returns:
            J: [batch, physics_state_dim, physics_state_dim] skew-symmetric matrix
        """
        J = torch.zeros(batch_size, self.physics_state_dim, self.physics_state_dim, 
                       device=self.device, dtype=J_elements.dtype)
        
        # Fill upper triangular (excluding diagonal)
        idx = 0
        for i in range(self.physics_state_dim):
            for j in range(i + 1, self.physics_state_dim):
    ...
    def _construct_R_matrix(self, R_tril_elements, batch_size):
        ...
        Returns:
            R: [batch, physics_state_dim, physics_state_dim] positive semi-definite matrix
        """
        # Create lower triangular matrix L
        L = torch.zeros(batch_size, self.physics_state_dim, self.physics_state_dim, 
                       device=self.device, dtype=R_tril_elements.dtype)
    ...
    def _compute_hamiltonian_gradient(self, state):
        """
        ...
        Returns:
            ...
            grad_H: [batch, physics_state_dim] gradient of H w.r.t. state
        """
        # Check if we're in no_grad mode (inference)
        if not torch.is_grad_enabled():
            # In inference mode, compute H and approximate gradient
            with torch.enable_grad():
                state_requires_grad = state.detach().clone().requires_grad_(True)
    ...
    def _compute_port_hamiltonian_dynamics(self, state):
        """
        ...
        H, grad_H = self._compute_hamiltonian_gradient(state)
    ...
    def forward(self, obs, rnn_states=None, masks=None, available_actions=None, deterministic=False):
        ...
        H, grad_H, _ = self._compute_port_hamiltonian_dynamics(state)
        ...
        physics_features = torch.cat([H, grad_H], dim=-1)  # [batch, 1 + physics_state_dim]
        ...
        # Compute log probability (per-dimension, like MAPPO)
    ...
    def evaluate_actions(self, obs, rnn_states, action, masks, available_actions=None, active_masks=None):
        ...
        # Compute log prob of given action (per-dimension, like MAPPO)
        action_log_probs = dist.log_prob(action)
...
# =============================================================================
# MAPPO-PINN Policy with Port-Hamiltonian Actor for MAMujoco
# =============================================================================

class MAPPO_PINN_Policy:
    """MAPPO policy with Port-Hamiltonian PINN Actor for MAMujoco environments."""
    ...
    def __init__(self, config, obs_space, cent_obs_space, act_space):
        ...
        # Use Port-Hamiltonian PINN Actor for physics-informed control
        self.actor = PortHamiltonianMujocoActor(config, self.obs_space, self.act_space, self.config["device"])
...
class Runner:
    ...
    def __init__(self,
        ...
        # Store num_agents in config for PINN Actor
    ...
    def run(self):
        ...
        eval_rewards=0.0
        eval_costs=0.0
        pbar = tqdm(range(episodes), desc="MAPPO-PINN-MuJoCo Training", ncols=120)
        for episode in pbar:
...
if __name__ == '__main__':
    set_np_formatting()
    args, cfg_env, cfg_train = multi_agent_args(algo="mappo_pinn_mujoco")

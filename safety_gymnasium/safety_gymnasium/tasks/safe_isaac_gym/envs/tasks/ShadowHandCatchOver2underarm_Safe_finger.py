...
class ShadowHandCatchOver2Underarm_Safe_finger(BaseTask):
    def __init__(
        ...
        self.num_obs_dict = {'openai': 42, 'full_no_vel': 77, 'full': 157, 'full_state': 422}
    ...
    def compute_cost(self):
        ...
        self.cost_buf = torch.where(
            actions[:, 2] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf
        )
        self.cost_buf = torch.where(
            actions[:, 2] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf
        )
        self.cost_buf = torch.where(
            actions[:, 3] < -0.5, torch.ones_like(self.cost_buf), self.cost_buf
        )
        self.cost_buf = torch.where(
            actions[:, 3] > 0.5, torch.ones_like(self.cost_buf), self.cost_buf
        )
    ...
    def compute_reward(self, actions):
        (
            ...
            self.hand_positions[self.hand_indices, :],
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[:, hand_pose_start : hand_pose_start + 3] = self.hand_positions[
            self.hand_indices, :
        ]
        ...
        self.obs_buf[:, action_another_obs_start : action_another_obs_start + 26] = self.actions[
            :, 26:
        ]
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            ...
            targets = (
                self.prev_targets[:, self.actuated_dof_indices + 24]
                + self.shadow_hand_dof_speed_scale * self.dt * self.actions[:, 32:52]
            )
        ...
        self.apply_forces[:, 1 + 26, :] = (
            self.actions[:, 26:29] * self.dt * self.transition_scale * 100000
        )
